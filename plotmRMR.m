...
plotmRMR_selectedvars_central(selected_scores, selectedvars, load_filename,dstcode);
...
% plot
originalfilename = "MRMR KS (selected features) district-specific";
savefilename = originalfilename;
plottitle = savefilename;

% Plot scores
figure('units', 'inches', 'outerposition', [0 0 8 8.5])
t = tiledlayout("flow")
...
for i = 1:length(dstcode_all)
    ...
    xtickangle(90)
    grid minor
    title("District " + string(dstcode))
end

xlabel(t,"Predictor rank")
ylabel(t,"Predictor importance score (Normalized)")
...
for i = 2:length(selected_scores)
    if selected_scores(i-1) - selected_scores(i) > 0.02
        selected_scores(i) = selected_scores(i) + 0.02;
    end
end
end


function plotmRMR_selectedvars_central(selected_scores, selectedvars, load_filename,dstcode)
originalfilename = "MRMR KS (selected features)";
savefilename = get_filenameext(originalfilename,load_filename,dstcode);
plottitle = savefilename;

% Plot scores
figure('units', 'inches', 'outerposition', [0 0 8 5])
t=tiledlayout("flow")
nexttile
bar(selected_scores)
xlabel(t,"Predictor rank")
